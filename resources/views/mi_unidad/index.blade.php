                ...
                <div class="flex items-center p-2 border rounded bg-gray-300 hover:bg-gray-100 transition ">
                    ...
                    <div class="flex items-center ml-auto space-x-2 relative">
                        ...
                        <div id="dropdown-menu-{{ $carpeta->id }}"
                            ...
                            <a href="#" class="px-4 py-2 text-gray-700 hover:bg-gray-100 flex items-center"
                                onclick="document.getElementById('eliminar-modal-{{$carpeta->id}}').style.display='flex'">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                    stroke="currentColor" class="size-6">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <!-- Se crea el Modela para editar el nombre d ela carpeta-->
                            <div id="eliminar-modal-{{$carpeta->id}}"
                                class="fixed inset-0 bg-black bg-opacity-50 items-center justify-center z-50 hidden"                                
                                onclick="if(event.target === this) this.style.display='none'">
                                <div class="bg-white p-6 rounded shadow-lg w-full max-w-md mx-auto flex flex-col">
                                    <h2 class="text-lg font-bold mb-4 text-gray-600">
                                        {{ __('Escribe el nombre de la carpeta a eliminar: ' )}}  {{$carpeta->nombre}}
                                    </h2>
                                    <form action="{{ route('mi_unidad.destroy', $carpeta->id) }}" method="POST">
                                        @csrf
                                        @method('delete')  
                                        <input type="hidden" name="carpeta" value="{{ $carpeta->id }}">
                                        <input type="text" name="nombre" id="input-nombre-{{ $carpeta->id }}"
                                                placeholder="Escribe el nombre de la carpeta para confirmar"
                                                class="border rounded px-3 py-2 w-full mb-4 text-gray-600" required
                                                oninput="validarNombreEliminar({{ $carpeta->id }}, '{{ addslashes($carpeta->nombre) }}')">

                                                <div id="error-nombre-{{ $carpeta->id }}" class="text-red-500 text-xs mb-2"></div>

                                                <div class="flex justify-end gap-2">
                                            <button type="button"
                                                onclick="document.getElementById('eliminar-modal-{{$carpeta->id}}').style.display='none'"
